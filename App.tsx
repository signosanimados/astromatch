...
const App: React.FC = () => {
  ...
  const fetchCredits = async (userId: string) => {
    ...
    try {
      ...
      if (data) {
        ...
        // Perfil não existe - criar com créditos iniciais
        ...
        if (newProfile) {
          setCredits(newProfile.credits);
           ...
           // Tenta buscar novamente em caso de race condition
           const { data: retryData } = await supabase
            ...
            if (retryData) setCredits(retryData.credits);
            else setCredits(0);
        }
      }
    } catch (e) {
  ...
  // Combinations Screen (original app logic)
  return (
    <div className="min-h-screen bg-[#050510] text-slate-200 pb-32 md:pb-20 font-sans relative overflow-x-hidden">
      ...
      <main className="container mx-auto px-4 md:px-8 relative z-10 pt-8">
        <header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 border-b border-white/5 pb-4">
          <div className="flex items-center gap-3">
             <button onClick={handleGoHome} className="flex items-center gap-3 hover:opacity-80 transition-opacity cursor-pointer">
               <img src={APP_LOGO} alt="AM" className="w-10 h-10 object-contain drop-shadow-[0_0_8px_rgba(255,255,255,0.5)]" crossOrigin="anonymous" />
               <h1 className="text-xl font-bold tracking-[0.2em] uppercase text-slate-300 hidden md:block">Signos Combinados</h1>
             </button>
          </div>

          <div className="flex items-center gap-4">
             <div className={`relative px-4 py-1.5 rounded-full flex items-center gap-3 shadow-lg transition-all border ${credits !== null && credits > 0 ? 'bg-slate-900 border-indigo-500/50 shadow-indigo-900/20' : 'bg-red-900/20 border-red-500/50 shadow-red-900/20 animate-pulse'}`}>
                <div className="flex flex-col items-end leading-none py-1">
                    ...
                    <span className={`text-lg font-mono font-bold ${credits === 0 ? 'text-red-400' : 'text-white'}`}>{credits ?? '-'}</span>
                </div>
                <div className="flex items-center gap-1">
                    <button onClick={() => session && fetchCredits(session.user.id)} className={`w-6 h-6 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/10 transition-all ${refreshingCredits ? 'animate-spin text-indigo-400' : ''}`} title="Atualizar saldo">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                    </button>
                    <button onClick={handleBuyCredits} className="w-6 h-6 bg-white text-slate-900 rounded-full flex items-center justify-center hover:bg-indigo-100 transition-colors font-bold text-sm pb-0.5 ml-1" title="Comprar mais créditos">+</button>
                </div>
             </div>
             <div className="h-8 w-[1px] bg-slate-800 hidden md:block"></div>
             <button onClick={handleGoHome} className="text-xs text-slate-500 hover:text-white transition-colors border border-slate-800 px-4 py-2 rounded-lg hover:bg-slate-800 flex items-center gap-2">
        ...
        {!result && (
          <div className="animate-fade-in max-w-6xl mx-auto flex flex-col items-center">
            <div className="flex flex-col items-center gap-3 mb-8 w-full">
              <span className="text-[10px] uppercase tracking-[0.2em] text-slate-500 font-bold">Tipo de Combinação</span>
              <div className="flex p-1 bg-slate-900/80 border border-slate-700/50 rounded-full shadow-lg relative">
                 <button onClick={() => setRelationshipMode('love')} className={`relative z-10 px-8 py-2 rounded-full text-sm font-bold tracking-widest transition-all duration-300 flex items-center gap-2 ${relationshipMode === 'love' ? 'bg-gradient-to-r from-pink-600 to-rose-500 text-white shadow-[0_0_15px_rgba(244,63,94,0.4)]' : 'text-slate-400 hover:text-slate-200'}`}>AMOR</button>
                 <button onClick={() => setRelationshipMode('friendship')} className={`relative z-10 px-8 py-2 rounded-full text-sm font-bold tracking-widest transition-all duration-300 flex items-center gap-2 ${relationshipMode === 'friendship' ? 'bg-gradient-to-r from-cyan-600 to-blue-500 text-white shadow-[0_0_15px_rgba(6,182,212,0.4)]' : 'text-slate-400 hover:text-slate-200'}`}>AMIZADE</button>
              </div>
            </div>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                 ...
                 <div className="flex-1 flex justify-center md:justify-end w-full md:w-auto">
                    <div className="flex flex-col items-end gap-3">
                        <button onClick={handleCalculate} disabled={!signA || !signB || loading} className={`w-full md:w-auto px-10 py-4 font-bold tracking-widest text-xs uppercase rounded-sm transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)] ${credits === 0 ? 'bg-indigo-600 text-white hover:bg-indigo-500 ring-2 ring-indigo-400 ring-offset-2 ring-offset-[#050510]' : 'bg-slate-100 text-slate-900 hover:bg-white disabled:opacity-20 disabled:cursor-not-allowed'}`}>
                          {loading ? 'Calculando...' : (credits !== null && credits <= 0 ? 'COMPRAR CRÉDITOS' : 'REVELAR')}
                        </button>
                        ...
                        {credits !== null && credits > 0 && <span className="text-[10px] text-emerald-400/70 font-mono">Custo: -1 crédito</span>}
                    </div>
                 </div>
              </div>
            </div>
        ...
        {result && signA && signB && (
          <ResultView result={result} signA={signA} signB={signB} mode={relationshipMode} onReset={handleReset} userEmail={session?.user?.email} />
        )}
      </main>
      ...
      {!result && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#050510]/95 backdrop-blur-xl border-t border-white/10 z-50 md:hidden flex items-center justify-between gap-4 shadow-[0_-5px_20px_rgba(0,0,0,0.8)]">
           ...
           <button onClick={credits === 0 ? handleBuyCredits : handleCalculate} disabled={!signA || !signB || loading} className={`flex-1 py-3 font-bold uppercase tracking-widest text-xs rounded-full shadow-lg transition-colors active:scale-95 ${credits === 0 ? 'bg-indigo-600 text-white animate-pulse' : 'bg-white text-slate-900 disabled:opacity-50 disabled:cursor-not-allowed'}`}>
              {loading ? '...' : (credits !== null && credits <= 0 ? 'Comprar (+)' : 'Revelar')}
           </button>
        </div>
      )}
    </div>
