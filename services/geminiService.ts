...
// ============================================================
// BIRTH CHART INTERPRETATION (AI POWERED - OpenAI GPT-4 Mini)
// ============================================================
...
/**
 * Generates a personalized birth chart interpretation using OpenAI GPT-4 Mini.
 */
export const generateBirthChartInterpretation = async (
  ...
  const jupiterSign = chart.planets.find(p => p.name === 'Júpiter')?.sign || 'desconhecido';
  const saturnSign = chart.planets.find(p => p.name === 'Saturno')?.sign || 'desconhecido';
  ...
  const elementName = dominantElement === 'fire' ? 'Fogo' :
                      dominantElement === 'earth' ? 'Terra' :
                      dominantElement === 'air' ? 'Ar' : 'Água';
  ...
  const modalityName = dominantModality === 'cardinal' ? 'Cardinal' :
                       dominantModality === 'fixed' ? 'Fixa' : 'Mutável';
  ...
  const planetsText = chart.planets.map(p =>
    `${p.name}: ${p.sign} (Casa ${p.house})${p.retrograde ? ' - Retrógrado' : ''}`
  ).join('\n');

  // Format main aspects
  const aspectsText = chart.aspects.slice(0, 10).map(a =>
    `${a.planet1} ${a.type} ${a.planet2} (${a.nature === 'harmonic' ? 'harmônico' : a.nature === 'challenging' ? 'desafiador' : 'neutro'})`
  ).join('\n');

  // Build the prompt
  const prompt = `Você é um astrólogo profissional brasileiro experiente. Gere uma interpretação completa e personalizada do mapa astral abaixo em português brasileiro.

${userName ? `Nome da pessoa: ${userName}` : 'Pessoa anônima'}

POSIÇÕES PLANETÁRIAS:
${planetsText}

ASCENDENTE: ${ascSign}
MEIO DO CÉU (MC): ${chart.midheaven.sign}

ASPECTOS PRINCIPAIS:
${aspectsText}

ELEMENTO DOMINANTE: ${elementName}
MODALIDADE DOMINANTE: ${modalityName}
ASPECTOS HARMÔNICOS: ${harmoniousAspects}
ASPECTOS DESAFIADORES: ${challengingAspects}

Instruções:
1. Comece com uma saudação personalizada${userName ? ` para ${userName}` : ''}.
2. Explique os "Três Grandes" (Sol, Lua e Ascendente) de forma profunda.
3. Analise os planetas pessoais (Mercúrio, Vênus, Marte).
4. Comente sobre o elemento e modalidade dominantes.
5. Destaque os aspectos mais importantes e seu significado.
6. Termine com uma mensagem inspiradora e motivacional.

Use emojis apropriados (✨🌟☀️🌙💫⭐) para tornar a leitura mais agradável.
Seja específico, evitando generalidades. A interpretação deve ter entre 800-1200 palavras.
Tom: acolhedor, místico, mas acessível e moderno.`;
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'Você é um astrólogo profissional brasileiro com décadas de experiência. Você cria interpretações de mapas astrais profundas, personalizadas e inspiradoras em português brasileiro.'
            },
            {
              role: 'user',
              content: prompt
  ...
  // Fallback to static interpretation if API fails or no key
  console.log('Using fallback interpretation (no API key or API error)');
...
// Fallback interpretation function (used when API is unavailable)
...
function buildFallbackInterpretation(params: FallbackParams): string {
  const {
    userName,
    sunSign,
    moonSign,
